...
const sampleEvents = [
  ...
  },
  {
    ...
    fullAgenda: 'Day 1: HTML/CSS/JS basics, Day 2: React fundamentals, Day 3: Backend with Node.js',
    faculty: 'IET',
    requiredBudget: 3000,
    fundingSource: 'GUC',
  },
  {
    ...
    fullAgenda: 'Introduction to Python, Pandas, Matplotlib, and real-world data analysis',
    faculty: 'MET',
    requiredBudget: 4000,
  ...
  // Trips
  {
    ...
    type: 'TRIP',
    description: 'Visit the historic city of Alexandria and explore its landmarks',
    location: 'Alexandria, Egypt',
    date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
    startDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
    endDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000 + 10 * 60 * 60 * 1000),
  ...
  // Conferences
  {
    ...
    fullAgenda: 'Keynote speakers, panel discussions, workshops, and networking sessions',
  ...
  },
  {
    ...
    fullAgenda: 'Expert talks on renewable energy, waste management, and green technology',
...
export async function seedComprehensiveData(): Promise<void> {
  ...
  try {
    ...
    for (const eventData of sampleEvents) {
      ...
      if (!existingEvent) {
        ...
        if (eventData.type === 'WORKSHOP') {
          createdBy = professors[Math.floor(Math.random() * professors.length)]?._id;
    ...
    const students = createdUsers.filter((u) => u.role === 'STUDENT' || u.role === 'STAFF' || u.role === 'TA');
    ...
    for (let i = 0; i < 5; i++) {
      ...
      console.log(`  ✓ Created reservation for ${randomStudent.firstName} at ${randomCourt.name}`);
    }
