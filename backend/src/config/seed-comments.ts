  ...
  try {
    ...
    await seedSampleComments();
        ...
        try {
          ...
          if (existing) {
            // Update existing with new comment
            await Feedback.updateOne(
              { _id: existing._id },
              {
                $set: {
                  ...
                  rating: commentData.rating,
                  type: 'both',
                  // Clear moderation fields so they get picked up by polling
                  moderationStatus: null,
                  moderationFlags: [],
                  moderationSeverity: null,
                  moderationConfidence: null,
                  moderationAiSuggestion: null,
                  moderationAiReasoning: null,
                },
              }
            );
            createdCount++;
    ...
    console.log(`\n✅ Seeded ${createdCount} comments across ${categories.length} categories`);
    ...
    console.log(`   Unmoderated: ${unmoderated}`);
    console.log(`\n🤖 The AI service will pick these up on next poll cycle.`);
