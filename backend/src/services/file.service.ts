...
const DANGEROUS_EXTENSIONS = [
  ...
  '.com', '.scr', '.pif', '.application', '.gadget', '.msi',
  '.msp', '.cpl', '.hta', '.inf', '.ins', '.isp', '.isu',
  '.job', '.lnk', '.msc', '.mst', '.paf', '.reg', '.rgs',
  '.scf', '.sct', '.shb', '.shs', '.u3p', '.vb', '.vbe',
  '.ws', '.wsf', '.wsh', '.dmg', '.pkg', '.app'
];
...
export class FileService {
  ...
  async uploadFile(options: UploadFileOptions): Promise<IFile> {
    ...
    console.log(`✓ File uploaded: ${filename} (${(processedFile.length / 1024).toFixed(2)}KB)`);
   ...
   */
  private async validateFileSignature(file: Buffer, mimeType: string): Promise<void> {
    ...
    if (!matches) {
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message: 'File content does not match declared type. Possible file tampering detected.',
      });
    }
  }
  ...
  private async compressImage(
    file: Buffer,
    mimeType: string
  ): Promise<{ buffer: Buffer; compressed: boolean; compressionRatio?: number }> {
    try {
      ...
      if (compressedSize < originalSize) {
        console.log(`✓ Image compressed: ${(originalSize / 1024).toFixed(2)}KB → ${(compressedSize / 1024).toFixed(2)}KB (${compressionRatio.toFixed(1)}% reduction)`);
        return {
          buffer: compressed,
          compressed: true,
          compressionRatio,
