...
export interface ConditionalAlert<
  TFieldValues extends GenericFormValues = GenericFormValues
> {
...
>({
  ...
  const form = useForm<TFieldValues>({
    ...
    defaultValues: (defaultValues ??
      fallbackDefaults) as DefaultValues<TFieldValues>,
  });
  ...
  const renderField = (fieldConfig: FormFieldConfig<TFieldValues>) => {
    ...
    if (fieldConfig.type === "custom" && fieldConfig.render) {
      const fieldValue = form.getValues(
        fieldConfig.name as FieldPath<TFieldValues>
      );
            ...
            <FormControl>
              {fieldConfig.type === "image" ? (
                <ImageUpload
                  ...
                  entityType={
                    fieldConfig.entityType === "vendor" ? "vendor" : "event"
                  }
                  aspectRatio="banner"
                  maxSizeMB={5}
                  disabled={isLoading}
            ...
            className={
              group.length > 1
                ? `grid grid-cols-1 md:grid-cols-2 ${
                    gridGap === 3 ? "gap-3" : gridGap === 5 ? "gap-5" : "gap-4"
                  }`
                : ""
            }
          >
        ...
        >
          ...
          >
            {isLoading ? (
              ...
              <>
                {submitButtonIcon && (
                  <span className="mr-2">{submitButtonIcon}</span>
                )}
                {submitButtonText}
              </>
            )}
            ...
            return (
              ...
              >
                ...
                >
                  {alert.icon && (
                    <div
                      className={cn("h-5 w-5 mt-0.5", alertIconColors[variant])}
                    >
                      {alert.icon}
                    </div>
                  )}
