...
export function EventsPage() {
  ...
  useEffect(() => {
    setPageMeta({
      title: "Events",
      description:
        "Browse workshops, trips, conferences, and more or manage your registrations and favorites",
    });
  }, [setPageMeta]);
  ...
  const handleTabChange = (tab: "browse" | "registrations" | "favorites") => {
    setActiveTab(tab);
    setPage(1);
  };
  ...
  const { data: favoriteEvents, isLoading: isFavoriteLoading } =
    trpc.events.getMyFavorites.useQuery(
      {
        page: 1,
        limit: 100,
      },
  ...
  const displayedEvents = useMemo(() => {
    ...
    if (activeTab === "favorites") {
      // Return favorite events from backend
      return (favoriteEvents as Event[] | undefined) || [];
    }

    // Browse tab - return what backend gives us
    return browseData?.events || [];
  }, [browseData, registrationsData, favoriteEvents, activeTab]);
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <EventFilters
        ...
        hidePrice={activeTab === "registrations" || activeTab === "favorites"}
      />
...
function EventCardProduction({
  ...
  const favoriteMutation = trpc.events.toggleFavorite.useMutation({
    onSuccess: () => {
      ...
      utils.events.isFavorite.invalidate({ eventId: eventId! });
      utils.events.getMyFavorites.invalidate();
    },
    onError: (error) => {
  ...
  const handleFavorite = async (e: React.MouseEvent) => {
    ...
    await favoriteMutation.mutate({ eventId });
    setIsFavoriting(false);
  };
        ...
        <div className="absolute top-3 right-3 flex gap-2">
          ...
          >
            <Heart
              fill={isFavorite?.isFavorite ? "red" : "none"}
              className="h-4 w-4"
              color={isFavorite?.isFavorite ? "red" : "currentColor"}
            />
          </Button>
        </div>
      </div>
